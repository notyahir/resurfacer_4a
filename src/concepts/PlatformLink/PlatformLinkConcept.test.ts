...
Deno.test("PlatformLinkConcept", async (t) => {
  ...
  await t.step("can action", async (t) => {
    ...
    await db.collection("PlatformLink.capabilities").insertOne({ linkId, capability: "play_music" });
  ...
  await t.step("Principle Trace: Full lifecycle of a platform link", async () => {
    ...
    await db.collection("PlatformLink.capabilities").insertOne({linkId, capability });
